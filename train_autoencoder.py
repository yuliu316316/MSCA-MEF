...
def train(i, original_imgs_path):
	...
	if args.resume is not None:
		...
		mef_model.load_state_dict(torch.load(args.resume))
	print(mef_model)
	optimizer = Adam(mef_model.parameters(), args.lr)
	...
	for e in tbar:
		...
		for batch in range(batches):
			...
			# get fusion image
			# encoder
			en = mef_model.encoder(img)
			# decoder
			outputs = mef_model.decoder_train(en)
			...
			if (batch + 1) % (200 * args.log_interval) == 0:
				...
				torch.save(mef_model.state_dict(), save_model_path)
				# save loss data
				# pixel loss
